...
wx.ready(function () {
  // 0.小程序接口
  // 0.1 跳转到index页面
  document.querySelector('#navigateToBtn').onclick = function () {
  ...
  // 0.2返回上一页
  document.querySelector('#navigateBackBtn').onclick = function () {
      wx.miniProgram.navigateBack({delta: 1});
  };
  // 0.3reLaunch
  document.querySelector('#reLaunchBtn').onclick = function () {
      wx.miniProgram.reLaunch({url: '/pages/index/index'})
  };
  // 0.4跳转到index页面，覆盖当前路径
  document.querySelector('#redirectToBtn').onclick = function () {
      wx.miniProgram.redirectTo({url: '/pages/index/index'})
  };
  // 0.5向小程序发送消息，会在特定时机（小程序后退、组件销毁、分享）触发组件的message事件 
  document.querySelector('#postMessageBtn').onclick = function () {
      wx.miniProgram.navigateBack({delta: 1});
  ...
  document.querySelector('#getEnvBtn').onclick = function () {
	...
	wx.miniProgram.navigateBack({delta: 1});
        wx.miniProgram.postMessage({data: resmini})
      });
  };
  ...
  document.querySelector('#previewImage').onclick = function () {
    wx.previewImage({
      current: 'https://mmbiz.qpic.cn/mmbiz_jpg/M6nr7uuNic7tU2yAMibJrW12C1bFPOo6LIZQYoZECxd70geBu77vmlicqXnEX5AtWPia5F4x04CFyB2bVCzsyHhWEw/0?wx_fmt=jpeg',
      urls: [
